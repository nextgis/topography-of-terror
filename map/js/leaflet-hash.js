		...
		update: function() {
			...
			if (parsed) {
				...
				var layers = parsed.layers,
					...
					baseLayer = options['main'],
					that = this;
				//console.log(layers);
				//Add/remove layers
				this.map.eachLayer(function(layer) {
					//var id = (layer._leaflet_id).toString();
					//if (layers.indexOf(id) == -1) { this.map.removeLayer(layer) };
					if (layer != baseLayer) { that.map.removeLayer(layer) };
				});

				layers.forEach(function(element, index, array) {
					//console.log(options[element]);
					if (element != 'main') { that.map.addLayer(options[element]) };
				});			
